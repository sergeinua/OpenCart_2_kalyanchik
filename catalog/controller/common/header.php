...
//		$data['text_logged'] = 'ololo';
		$data['text_account_head'] = !$this->customer->isLogged() ? $this->language->get('text_account') : $this->customer->getFirstName();
...
//		$this->load->model('tool/image');
		...
		if (isset ($this->request->get['path'])) {
			...
			$img = $this->model_catalog_category->getCategory($data['cat_id']);
			$this->load->model('tool/image');
			if($img) {
				if ($this->model_tool_image->resize($img['image'], 100, 100))
					$data['category_image'] = $this->model_tool_image->resize($img['image'], 100, 100);
			}
