		...
		if ($results) {
            foreach ($results as $result) {
                ...
                if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                    ...
                    $dollar_price =  $this->currency->format($this->tax->calculate($result['price'], $result['tax_class_id'], $this->config->get('config_tax')));
                ...
                if ((float)$result['special']) {
                    ...
                    $dollar_special = $this->currency->format($this->tax->calculate($result['special'], $result['tax_class_id'], $this->config->get('config_tax')));
                ...
                $data['products'][] = array(
                    'discount'		=> ($price && $price!=0 && $special) ? round(($price - $special)*100/($price)) : false,
