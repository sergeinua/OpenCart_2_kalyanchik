		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			foreach ($products as $product) {
				...
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                    ...
                    $dollar_price =  $this->currency->format($priceint,"USD");
                     $price = sprintf($this->language->get('text_price_with_dollar'), $price,$dollar_price);
				...
				// Display prices
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                    $totalint = $this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity'];
					$total = $this->currency->format($totalint);
                    $dollar_price = $this->currency->format($totalint,"USD");
                    $total =  sprintf($this->language->get('text_price_with_dollar'), $total,$dollar_price);
				...
				$data['products'][] = array(
					'discount'		=> ($price && $total && $price!=0) ? round(($price - $total)*100/($price)) : false,
			...
			foreach ($total_data as $total) {
                $price = $this->currency->format($total['value']);
                $dollar_price = $this->currency->format($total['value'],'USD');
                $price = sprintf($this->language->get('text_price_with_dollar'), $price,$dollar_price);
				$data['totals'][] = array(
					'title' => $total['title'],
					'text'  => $price
