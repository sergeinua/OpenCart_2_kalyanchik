            ...
            foreach ($this->session->data['cart'] as $key => $quantity) {
                ...
                if ($product_query->num_rows) {
                    ...
                    if ($product_special_query->num_rows) {
                        ...
                        $quantity_m = $query1->row['quantity'];
                        ...
                        $q1 = "SELECT cd.* FROM " . DB_PREFIX . "common_discount_to_product cd2p  LEFT JOIN " . DB_PREFIX . "common_discount cd ON cd2p.common_discount_id = cd.common_discount_id WHERE product_id = '" . (int)$product_id . "' AND cd.status = '1' AND cd.count <= '" . (int)$quantity_m . "' AND cd.common_discount_id IN (" . $q4 . ")";
                        $q2 = "SELECT cd2.* FROM " . DB_PREFIX . "common_discount_to_manufacturer cd2m  LEFT JOIN " . DB_PREFIX . "common_discount cd2 ON cd2m.common_discount_id = cd2.common_discount_id WHERE manufacturer_id = '" . (int)$manufacturer_id . "' AND cd2.status = '1' AND count <= '" . (int)$quantity_m . "' AND cd2.common_discount_id IN (" . $q4 . ")";
                        $q3 = "SELECT cd3.* FROM " . DB_PREFIX . "common_discount_to_category cd2c  LEFT JOIN " . DB_PREFIX . "common_discount cd3 ON cd2c.common_discount_id = cd3.common_discount_id WHERE category_id IN (SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "') AND cd3.status = '1' AND count <= '" . (int)$quantity_m . "' AND cd3.common_discount_id IN (" . $q4 . ")";

                        $query  = $this->db->query($q1 . " UNION ". $q2 . "UNION " . $q3 . " ORDER BY priority DESC");
                        if (isset($query->row['percent'])) {
                            $price = $price * (1 - $query->row['percent'] / 100);
                        }
                    }
