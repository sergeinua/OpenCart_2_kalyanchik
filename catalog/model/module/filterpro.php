...
class ModelModuleFilterPro extends Model {
	...
	public function __construct($registry) {
        ...
        $this->filterpro_setting = $this->getModulePreferences();
...
//		}
    ...
    public function getModulePreferences(){
        $sql = "SELECT * from ". DB_PREFIX . "module where `code` = 'filterpro' LIMIT 1";
        $query = $this->db->query($sql);
        $res = unserialize($query->rows[0]['setting'])['filterpro_setting'];
...
//		}
		...
		if($data['attribute_value']) {
			if($this->filterpro_setting['attribute_mode'] == 'and') {
				$i = 0;
				foreach($data['attribute_value'] as $attribute_id => $values) {
					$sql .= " AND EXISTS (select 1 FROM " . DB_PREFIX . "product_attribute p2a" . $i .
							" WHERE p2a" . $i . ".product_id=p2a.product_id AND p2a" . $i . ".attribute_id = " . (int)$attribute_id . " ) ";
				}
			}
		}
