...
$(document).on("change",'.filtered', (function () {
    iF();
}));
$(function () {
    $("#scale-price").slider({range:true, min:0, max:0, values:[0, 0], stop:function (a, b) {
        iF()
    }, slide:function (a, b) {
        $("#min_price").val(b.values[0]);
        $("#max_price").val(b.values[1])
    }});
    $("#min_price").val($("#scale-price").slider("values", 0));
    $("#max_price").val($("#scale-price").slider("values", 1))
});
function iF() {
    clearTimeout(fIID);
...
view = 'grid';
    if (g.result) {
        element_to_append =  element_to_append||$(".product-" + view).parent();
        element_to_append.empty();
        console.log(".product-" + view);
        

        $.each(g.result,function(i,v){
            switch(view){
                case "grid":
                    v.view = "product-grid col-lg-4 col-md-4 col-sm-6 col-xs-12 asd"
                    break;
                case "list":
                    v.view = "product-list col-xs-12";
            }
        });
        console.log(g.result)
        $("#productTemplate").tmpl(g.result).appendTo(element_to_append); 
    }
    $("#pagination-block").html(g.pagination);
    if (b && g.min_price == g.max_price) {
        $('.price_slider').hide();
        $('.filterpro-module').hide();
    ...
    if (b) {
        $("#scale-price").slider("option", {min:d, max:c});
        if ($("#max_price").val() >= 1) {
            d = parseInt($("#min_price").val());
            c = parseInt($("#max_price").val())
        }
        $("#scale-price").slider("option", {values:[d, c]});
        $("#min_price").val(d);
        $("#max_price").val(c)
    }
    if (g.totals_data) {
...
//    }
    ...
    $(".clear_filter").click(function () {
        ...
        var b = $("#scale-price").slider("option", "min");
        var a = $("#scale-price").slider("option", "max");
        $("#scale-price").slider("option", {values:[b, a]});
        $("#min_price").val(b);
        $("#max_price").val(a);
	$("div[id^=scale-price-]").each(function(index, element) {
...
// deserialize
    ...
    if ($("#input-sort").length) {
        if ($("#filterpro_sort").val()) {
            $("#input-sort").val($("#filterpro_sort").val() + "&" + $("#filterpro_order").val())
        } else {
            vars = $("#input-sort").val().split("&");
            $("#filterpro_sort").val(vars[0]);
            $("#filterpro_order").val(vars[1])
        }
    }
    if ($("#filterpro_limit").length) {
        if ($("#filterpro_limit").val()) {
            $("#input-limit").val($("#filterpro_limit").val())
        } else {
            $("#filterpro_limit").val($("#input-limit").val())
        }
    }
    doFilter(true)
});
