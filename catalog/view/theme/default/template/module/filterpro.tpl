...
<div id="offcanvas"  class="filterpro-module shadow-block">

    <button type="button" id="close-offcanvas" class="visible-xs btn-link" href="" role="button">
        ×
    </button>
...
<div class="price_slider" <?php if(!$price_slider) { echo 'style="display:none"';}?>>
    ...
    <div class="min_price">
        ...
        <input class="price_limit form-control input-form" type="text" name="min_price" value="-1" id="min_price"/>
    </div>
    <div class="max_price">
        <label for="min_price">до</label>
        <input class="price_limit form-control input-form" type="text" name="max_price" value="-1" id="max_price"/>
    </div>
    ...
    <table>
		<tr>
			...
			<?php if($symbol_left){ echo "<td><b>".$symbol_right."</b></td>";}?>
		</tr>
	</table>
...
<div class="option_box">
	<div class="option_name filter-title<?php if(!$expanded_categories){echo "hided";}?>"><?php echo $text_categories; ?></div>
	...
	<?php if($tags) { ?>
<div class="option_box">
	<div class="option_name filter-title<?php if(!$expanded_tags){echo "hided";}?>"><?php echo $text_tags; ?></div>
	...
	<?php if($manufacturers) { ?>
<div class="option_box">
	<div class="option_name filter-title<?php if(!$expanded_manufacturer){echo "hided";}?>"><?php echo $text_manufacturers; ?></div>
	...
	<?php } ?>
		...
		<div class="attribute_box <?php if($attr_group=="0") echo "option_box"; ?>">
			...
			<div class="option_name filter-title <?php if(!$attribute_value['expanded']){echo "hided";}?>"><?php echo $attribute_value['name']; ?></div>
			<div class="collapsible" <?php if(!$attribute_value['expanded']){echo 'style="display:none"';}?>>
				<?php if($attribute_value['display'] == 'select') { ?>
				<div class="my-select">
					<select class="form-control filtered" name="attribute_value[<?php echo $attribute_value_id?>][]">
						<option value=""><?php echo $text_all?></option>
	...
	<?php foreach($options as $option) { ?>
	<div class="option_box">
		<div class="option_name filter-title <?php if(!$option['expanded']){echo "hided";}?>"><?php echo $option['name']; ?></div>
		<?php if($option['display'] == 'select') { ?>
		<div class="collapsible" <?php if(!$option['expanded']){echo 'style="display:none"';}?>>
			<select class="form-control filtered" name="option_value[<?php echo $option['option_id']?>][]">
				<option value=""><?php echo $text_all?></option>
				<?php foreach($option['option_values'] as $option_value) { ?>
		...
		<div class="" <?php if(!$option['expanded']){echo 'style="display:none"';}?>>
			<?php foreach($option['option_values'] as $option_value) { ?>
					<div class="my-checkbox">
                        <input class="one filtered option_value" id="option_value_<?php echo $option_value['option_value_id']?>"
                               type="checkbox" name="option_value[<?php echo $option['option_id']?>][]"
                               value="<?php echo $option_value['option_value_id']?>">
                        <label for="option_value_<?php echo $option_value['option_value_id']?>"><?php echo $option_value['name']?>
                        </label>
                    </div>
                        <?php } ?>
		</div>
...
<!-- <button type="submit" class="btn filter-btn">Подобрать</button> -->
</div>
</div>
</form>
	<script>
                            ...
                            <div class="module-good product-thumb">
                                ...
                                {{if thumb}}
                                    ...
                                    <span class="image-link">
                                        <img src="${thumb}"
                                             alt="${name}"
                                             title="${name}" class="img-responsive">
                                                </span>
                                ...
                                <div class="caption">
                                    ...
                                    <div class="good-rating">
										...
										{{if reviews}}
											<a href="${href}#review-block">
												<div class="list-reviews-count">${reviews}</div>
											</a>
										{{/if}}
									</div>
